...
type
  ...
  public
    ...
    procedure DefinitDimensionsParDefaut; // v1.8.2 : plus de paramètres
  end;
//---------------------------------------------------------------------------
...
implementation
//---------------------------------------------------------------------------
uses Math, LCLType, main_f, fdmatch_f;
//---------------------------------------------------------------------------
{$R *.lfm}
//---------------------------------------------------------------------------
...
try
  ...
  Top:=FormMain.Top; // v1.8.3 : le tirage a bougé après retaille !
  Left:=FormMain.Left+FormMain.Width; // v1.8.3 : le tirage a bougé après retaille !
  if FormFeuilleMatch.Visible then FormFeuilleMatch.FormShow(self); // v1.8.3 : on réaffiche la feuille de match (la position va changer)
end{try}
end;
//---------------------------------------------------------------------------
...
begin // v1.4.1
// Application.MessageBox(pChar(IntToStr(Key)), 'KeyDown');
...
Repaint // vLaz : Repaint remplace DessineTirage
end;
//---------------------------------------------------------------------------
...
var i              : Integer;
    ...
    NumeroJeton    : TNumeroJetonSac;
begin // v1.4.1
// Application.MessageBox(pChar('Key '+Key), 'KeyPress');
...
if ChoixJeton=0 then ChoixJeton:=1;
...
with Canvas do
  ...
  for i:=Low(TOrdreJetonChevalet) to High(TOrdreJetonChevalet) do
    if Chevalet[i]>0 then
      begin // /!\ ATTENTION LE DESSIN DES JETONS EST DUPLIQUÉ DANS FormMain.DessineCase /!\
      // Ensuite les jetons
      x1:=1+TailleCase*(i-1);
      y1:=1;
      x2:=x1+TailleCase;
...
//---------------------------------------------------------------------------
procedure TFormTirage.DefinitDimensionsParDefaut; // v1.8.2 : plus de paramètres
begin
...
FChoixDebut:=Debut;
FChoixFin:=Fin;
if Visible then Repaint // vLaz : Repaint remplace DessineTirage
end;
//---------------------------------------------------------------------------
