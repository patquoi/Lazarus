...
implementation
//----------------------------------------------------------------------------
uses Math, LCLType, base, main_f, fdmatch_f, tirage_f, prmpose_f, definition_f;
//---------------------------------------------------------------------------
{$R *.lfm}
//----------------------------------------------------------------------------
...
if FormFeuilleMatch<>nil then
  begin
  if FormFeuilleMatch.Visible then // v1.8.8 : on aligne avaec la feuille de match
    begin
    Top:=FormFeuilleMatch.Top;
    Left:=FormFeuilleMatch.Left;
    end;
...
if FormDefinitionMot<>nil then // v1.6 : propose le mot pour la recherche de définition (Maj+Ctrl+D)
  FormDefinitionMot.EditMot.Text:=LowerCase((FormSolutions.ListBox.Items.Objects[DernierChoix] as TSolution).stMot);
end;
//----------------------------------------------------------------------------
...
if slSolutions.Count=1 then // v1.5 : on n'ajoute pas la position de choix si qu'une seule solution
  ListBox.Items.AddObject(slSolutions.Strings[0], slSolutions.Objects[0])
else
  begin
...
if slSolutions.Count=1 then // v1.4 : On a déplacé ici le cas solution unique pour pouvoir annuler la pose de la meilleure solution
  ...
  ListBox.ItemIndex:=0;
  if FormDefinitionMot<>nil then // v1.6 : propose le mot pour la recherche de définition (Maj+Ctrl+D)
    FormDefinitionMot.EditMot.Text:=LowerCase((FormSolutions.ListBox.Items.Objects[FormSolutions.ListBox.ItemIndex] as TSolution).stMot);
