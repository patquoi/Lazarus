...
implementation
//---------------------------------------------------------------------------
uses IniFiles, LCLIntf, base, main_f, prmsrcdef_f;
//---------------------------------------------------------------------------
{$R *.lfm}
//---------------------------------------------------------------------------
...
begin
if NbSrcDefPerso=0 then // vLaz : OpenURL pour ouvrir une page web
   OpenURL(Format(stFrmURLSrcDef[TSourceDefinitions(ComboBoxSource.ItemIndex)], [EditMot.Text]))
else
   OpenURL(Format(stFrmURLSrcDefPerso[ComboBoxSource.ItemIndex], [EditMot.Text]))
end;
//---------------------------------------------------------------------------
...
try
  ...
  // 2. Chargement de la source de définitions sauvegardées dans la section [Paramètres] du fichier Duplicata.ini
  try
