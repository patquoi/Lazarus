...
interface
//----------------------------------------------------------------------------
uses Classes, Graphics, LCLIntf, LCLType, IniFiles;
//----------------------------------------------------------------------------
const NbCasesCote         =  15;
   ...
   // MinutesReflexion    = 3; // v1.5.6. Obsolète. Utilisation de TPartie.TempsReflexion à la place
      ...
      stHTMLSuffixeRang   : array [0..1] of String = ('er', '&egrave;me'); // vLaz : pour feuille de match HTML détaillée
...
type  TBonus = (bAucun,
     ...
     //-----------------------------------------------------------------------
      ...
      public
        ...
        // fonctions de stockage. Pas d'extension : .dup => Partie. .rtf => Feuille de match en format rtf.
...
const CoefBonusMot    : array [TBonus] of Integer = (1,1,1,2,3);
      ...
      ValeurJeton   : array [TTypeJeton]   of Integer   = (0,0,1,3,3,2,1,4,2,4,1,8,10,1,2,1,1,3,8,1,1,1,1,4,10,10,10,10);
      ...
      TypeJeton : array [TNumeroJetonSac]    of TTypeJeton = (tjIndefini,
                                                              tjJoker, tjJoker, tjA, tjA, tjA, tjA, tjA, tjA, tjA, tjA, tjA, tjB, tjB, tjC, tjC, tjD, tjD,
                                                                  tjD,     tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjE, tjF,
                                                                  tjF,     tjG, tjG, tjH, tjH, tjI, tjI, tjI, tjI, tjI, tjI, tjI, tjI, tjJ, tjK, tjL, tjL,
                                                                  tjL,     tjL, tjL, tjM, tjM, tjM, tjN, tjN, tjN, tjN, tjN, tjN, tjO, tjO, tjO, tjO, tjO,
                                                                  tjO,     tjP, tjP, tjQ, tjR, tjR, tjR, tjR, tjR, tjR, tjS, tjS, tjS, tjS, tjS, tjS, tjT,
                                                                  tjT,     tjT, tjT, tjT, tjT, tjU, tjU, tjU, tjU, tjU, tjU, tjV, tjV, tjW, tjX, tjY, tjZ);

      GenreJeton : array [TTypeJeton]      of TGenreLettre = (glIndefini, glIndefini, // Y et joker considéré comme indéfini !
                                                              glVoyelle,  glConsonne, glConsonne, glConsonne, glVoyelle,  glConsonne, glConsonne, glConsonne,
                                                              glVoyelle,  glConsonne, glConsonne, glConsonne, glConsonne, glConsonne, glVoyelle,  glConsonne, glConsonne,
                                                              glConsonne, glConsonne, glConsonne, glVoyelle,  glConsonne, glConsonne, glConsonne, glIndefini, glConsonne);
      ...
      stVersionDico                                   = 'ODS9'; // modifié v1.10 (9) // modifié v1.9 (8) // modifié v1.8 (7) // créé v1.5.3
      ...
      stHTMLNatureRecord      : array [TNatureRecord] of String = ('Score', // vLaz : version HTML
                                                                   'Nb Bonus 50',
                                                                   'Taille de mot form&eacute;',
                                                                   'Niv.de diff.',
                                                                   'Rapport Nb Sol./Rang', // x100
                                                                   'Nb solutions',
                                                                   'Nb sol. avec bonus', // x100
                                                                   'Rapport Score/Top',
                                                                   'Nb tops trouv&eacute;s',  // x100
                                                                   'Tps de calc. des sol.', // v1.6
                                                                   'Nb sol. trouv&eacute;es / s.'); // v1.6
      ...
      // v1.5.3 : Gestion du défi : Mot de 15 lettres donné posé le plus rapidement possible
...
//---------------------------------------------------------------------------
uses
  SysUtils, Math, StdCtrls, StrUtils,
...
procedure OuvreFichierHTML(stCheminFichier : String); // v1.4.6
begin
 OpenURL(stURLCheminNomFichier(stCheminFichier)) // v1.5 : utilise la fonction ci-dessus. vLaz : utilise OpenURL
end;
//---------------------------------------------------------------------------
function stTirageTrie(const stTirage : String) : String;
var i, j : Integer;
...
const stFrmHTMLEnTete = '<?xml version="1.0" encoding="utf-8"?>'#13+
                        '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">'#13+
                        '<html><head><title>Solutions tri&eacute;es - Duplicata</title><meta content="Solutions tri&eacute;es Duplicata" name="description"><meta content="Solutions tri&eacute;es, Duplicata" name="keywords"></head>'#13+
                        '<body text="#fffff" vlink="#a0a0ff" link="#00ffff" bgcolor="#000000">'#13+
                        '<h1>Solutions tri&eacute;es - Duplicata</h1>'#13;
      stFrmHTMLPdPage = '</tbody></table>'#13#13'</body></html>';
      stFrmHTMLETPlt  = '<table cellspacing="1" cellpadding="2" border="1"><tbody>'#13+'<tr><td>&nbsp;</td>';
      // v1.4.8 : Ajout du nombre de solutions distinctes affichées dans le tablau
      stFrmHTMLPdPPlt = '</tr>'#13'</tbody></table><BR>'#13#13'Tirage au tour n&deg;%d&nbsp;: <B>%S</B>. Nombre de solutions distinctes : %d.<P><U>Remarque</U> : Les lettres en minuscules indiquent les jokers utilis&eacute;s.<BR>'#13#13; // v1.4.1 (Rq)
      stFrmHTMLETSol  = '<table cellspacing="1" cellpadding="2" border="1"><tbody>'#13+
                        '<tr><td align="center" colspan="3"><I>Crit&egrave;res de tri</I></td><td align="center" colspan="3"><I>Solutions</I></td></tr>'#13+
                        '<tr><td><I>Score</I></td><td><I>Jeton(s)<BR>plac&eacute;(s)</I></td><td><I>Valeur<BR>plac&eacute;e</I></td><td><I>Rang</I></td><td><I>Position(s)<BR>1er jeton du mot</I></td><td><I>Mot(s) pos&eacute;(s)</I></td></tr>'#13; // v1.7.2. "1er jeton du mot" au lieu de "1er jeton posé"
...
var   f                : TextFile;
  ...
  for y:=Low(TCoordonnee) to High(TCoordonnee) do
    begin
    Write(f, '</tr>'#13'<tr><td><FONT COLOR="#7F7F7F"><I>'+stLettreJeton[TTypeJeton(y+2)]+'</I></FONT></td>');
    for x:=Low(TCoordonnee) to High(TCoordonnee) do
      if p.c[x,y]>0 then
        begin
        stLettre:=p.stLettre(x, y);
        if TypeJeton[p.c[x,y]]=tjJoker then
          stLettre:=LowerCase(stLettre);
        Write(f, Format('<td align="center"><FONT COLOR="%s"><B>%s</B></FONT></td>', [stCodeHTMLBonus[BonusCase[x,y]], stLettre]))
        end
      else
  ...
  stTirage:=Copy(AnsiReplaceStr(p.stTirage[p.Tour],' ','?'), 1, p.NbJetonsAJouer); // v1.3.8 (Copy)
...
stNomFichierHTML:=ExtractFilePath(ParamStr(0))+'Solutions.html';

(* // Trace
AssignFile(f, ExtractFilePath(ParamStr(0))+'\Trace.txt');
Rewrite(f);
  ...
  for i:=0 to slSolutionsTriees.Count-1 do
    ...
    if LignesAgregeesAVenir=0 then
      ...
      while (i+di<slSolutionsTriees.Count) and (di<=Agregats) and (stPosition=Copy(slSolutionsTriees[i+di], PosPosition, TaillePosition)) do
        begin
        stSolutions:=stSolutions+' '+Copy(slSolutionsTriees[i+di], PosSolution, Length(slSolutionsTriees[i+di])-PosSolution+1);
...
function stURLBitmapNiveauDifficulte(const Difficulte : TDifficulteTirage) : string;
begin
Result:=stURLCheminNomFichier(ExtractFilePath(ParamStr(0))+Format('html/%d.png', [Ord(Difficulte)])) // v1.8.4 : png au lieu de bmp
end;
//---------------------------------------------------------------------------
...
stURLBitmap:=stURLCheminNomFichier(ExtractFilePath(ParamStr(0))+'html/'+IntToStr(Ord(Difficulte))+'.png') // v1.8.4 : png au lieu de bmp
end;
//---------------------------------------------------------------------------
function stDateHeureCourante : String; // v1.5
var dt : TDateTime;
...
var
  ...
  k, l, o : Word;
  n : AnsiChar;
  stTirage : AnsiString; // RX         TStream
...
try
  with FormPatience do
    ...
    ProgressBar.Max:=High(TNbLettres);
    ProgressBar.Min:=Low(TNbLettres)-1;
  ...
  for i:=Low(TNbLettres) to High(TNbLettres) do
    ...
    for k:=1 to j{NbTirages} do
      ...
      // v1.6.7 - Dico ODS6, code ci-dessous obsolète
      (*
      if stTirage=#0#0'CCEEHIINNOOTZ' then
        if stTirage='CCEEEHIINNNOOTZ' then
          stTirage:='CCEEEHIINNNOOTZ'; // v1.5.2 : entrée incorrecte correspondant à l'entrée incorrecte du dernier mot du dico ZOOTECHNICIENN~E : On réintègre le E.
      *)
      Tirages[i].AddObject(stTirage, Anagrammes);
      end{for j};
    with FormPatience do
      begin
...
try
  with FormPatience do
    ...
    ProgressBar.Max:=(NbLettresDico div 5)-1; // v1.8 (ODS7)
    ProgressBar.Min:=0;
    ProgressBar.Position:=ProgressBar.Min;
    Show;
    Panel.Repaint;
  ...
  for i:=0 to (NbLettresDico div 5)-1 do // v1.8 (ODS7)
    begin
    ...
    if ((i=(NbLettresDico div 5)-1) or
        (i mod (NbLettresDico div 500)=0)) then // v1.8 (ODS7)
      with FormPatience do
        begin
        ProgressBar.Position:=i;
        ProgressBar.Repaint;
...
begin // Paramètre = N° de jeton physique et non n° d'ordre dans le sac
if TypeJeton[NumeroJeton]=tjJoker then
  Result:=LowerCase(stLettreJeton[LettreJoker[NumeroJeton]])
else
  Result:=stLettreJeton[TypeJeton[NumeroJeton]]
end;
//---------------------------------------------------------------------------
...
for iSelection:=Debut to Fin do
  if Chevalet[iSelection]>0 then
    ...
    if not MotForme90Valable(X+iPlateau*dx[d],
      ...
      if TypeProposition<>tpRecherche then
        begin
        stMessage:=stMessage+IfThen(NbMotsNonValables=0, '', ', ')+stMot90; // v1.6 : On place le texte après avoir définil la liste de mots non valables
...
if not Dico.Existe(stMot) then
  ...
  if TypeProposition<>tpRecherche then
    ...
    stMessage:=stMot+IfThen(NbMotsNonValables=0, '', ', ')+stMessage;
    Inc(NbMotsNonValables);
    end;
  end;
if NbMotsNonValables>0 then // v1.6 : on place le texte mot(s) non valide(s)
  // vLaz : Format bogué ! Il oublie le reste de la chaîne stMsgMotNonValable à savoir "n'est pas/ne sont pas valable(s)" !
  // stMessage:=Format(stMsgMotNonValable[NbMotsNonValables>1], [stMessage]);
  stMessage:=StringReplace(stMsgMotNonValable[NbMotsNonValables>1], '%s', stMessage, []);
...
//---------------------------------------------------------------------------
// fonctions de stockage. extension .dup => Partie ; .rtf => Feuille de match en format rtf.
//---------------------------------------------------------------------------
...
try
  ...
  for i:=Low(TTour) to High(TTour) do
    ...
    for j:=1 to k do
      BlockWrite(f, stProposition[i,j], 1); // v1.8.3 : caractères stockés en 8 bits
    k:=Length(stSolution[i]);
    BlockWrite(f, k, SizeOf(k));
    for j:=1 to k do
      BlockWrite(f, stSolution[i,j], 1); // v1.8.3 : caractères stockés en 8 bits
    k:=Length(stTirage[i]);
    BlockWrite(f, k, SizeOf(k));
    for j:=1 to k do
      BlockWrite(f, stTirage[i,j], 1); // v1.8.3 : caractères stockés en 8 bits
    end;
  if VersionFichier>=$156 then // v1.5.6
    begin
...
try
  ...
  if (VersionFichier<>$140) and
    ...
    stMessage:='Ce n''est pas un fichier de partie Duplicata ou le fichier a été généré avec une version de Duplicata antérieure à la version 1.4.';
    Exit
    end;
  if VersionFichier>VersionFichierCrt then // v1.5
    begin
    stMessage:='Cette partie a été enregistrée avec une version ultérieure à la version actuelle de Duplicata.';
    Exit
    end;
  for i:=Low(TCoordonnee) to High(TCoordonnee) do
    for j:=Low(TCoordonnee) to High(TCoordonnee) do
  ...
  for i:=Low(TTour) to High(TTour) do
    ...
    for j:=1 to k do
      begin
      BlockRead(f, l[1], 1); // v1.8.3 : caractères stockés en 8 bits
...
var x, y,
  ...
  var i, j, k,
    ...
    function ChoixJetonsAPlacer : Boolean; // v1.4.1. Retourne vrai s'il on a trouvé encore une possibilité de choisir et fait le choix (place les jetons sur le chevalet et choisit entre jokers et lettres)
      ...
      if TypeJeton[t[OrdreJeton]]=tjJoker then
        stAnagrammeAJ[PositionMot]:=LowerCase(stAnagramme)[PositionMot]
      else
        stAnagrammeAJ[PositionMot]:=UpperCase(stAnagramme)[PositionMot];
      ...
      //-----------------------------------------------------------------------
      function ChangeChoixLettreJoker(PositionMot : ShortInt) : Boolean; // Retourne Faux si plus de choix. vLaz : ShortInt au lieu de TPositionMot
      var j, k,
      ...
      //-----------------------------------------------------------------------
      function ChercheChoix(PositionMot : ShortInt) : Boolean; // Retourne Faux si plus de choix. vLaz : ShortInt au lieu de TPositionMot
      var OrdreJeton : TOrdreJetonTirage;
      begin
      Result:=False; // Pessimiste
      ...
      //-----------------------------------------------------------------------
      function ChercheChoixSuivant(PositionMot : ShortInt) : Boolean; // Retourne Faux si plus de choix. vLaz : ShortInt au lieu de TPositionMot
      var OrdreJeton : TOrdreJetonTirage;
      begin
      Result:=False; // Pessimiste
  ...
  Journalise(SansSautDeLigne, Format('      %s', [stTirageComplet]));
  ...
  for i:=1 to NbJetonsAPlacer do
    while CoordonneesValides(x+(i+NbJetonsPlacesApres)*dx[d],
      ...
      Journalise(SansSautDeLigne, Format('%s', [stLettrePlacee[NbJetonsPlacesApres]]));
  ...
  while CoordonneesValides(x+PositionDebutMot*dx[d],
    ...
    Journalise(SansSautDeLigne, Format('%s', [stLettrePlacee[PositionDebutMot]]));
  ...
  Journalise(AvecSautDeLigne, Format('=>%s', [stTirageCompletTrie]));
  ...
  if Taille>=Low(TNbLettres) then
    ...
    if iTirageRecherche>-1 then
      ...
      for j:=0 to Anagrammes.NbAnagrammes-1 do
        ...
        Journalise(SansSautDeLigne, Format('        %s: ', [stAnagramme]));
        k:=0;
        ...
        if MotOk then
          ...
          While ChoixJetonsAPlacer do // v1.4.1 : le choix des jetons à placer a été déplacé dans la fonction ChoixJetonsAPlacer qui retourne s'il y a encore un choix possible (gestion des choix jokers/lettre)
            begin
            Journalise(SansSautDeLigne, '          '+stAnagrammeAJ); // v1.4.1
            ...
            if MotsFormesValables(x, y, d, Chevalet, Low(TOrdreJetonChevalet), NbJetonsAPlacer, tpRecherche) then // v1.4 : tpRecherche
              ...
              if (NbJetonsAPlacer=1) then
                if d=dVerticale then // v1.4.3 : Nouvelle façon de tester les doublons de solutions à 1 jeton placé : on teste la solution verticale à la solution horizontale stockée
                  ...
                  stDrnSol1J[stLettreAJ(Chevalet[1])[1]]:=Format(stFrmSolution,
                                                                 [stAnagrammeAJ, stDirection[d], stCoordonnees(x, y, d)]); // v1.4.1 +AJ
                  end;
              ...
              if ScoreSol>=ScoreTop[Tour] then
                begin
                stSolution:=Format(stFrmSolution,
                                   [stAnagrammeAJ, stDirection[d], stCoordonnees(x, y, d)]); // v1.4.1 +AJ
  ...
  Journalise(AvecSautDeLigne, '');
  if NbJokers>0 then
    for Lettre1:='A' to 'Z' do
      begin
      stTirage[iJoker[1]]:=Lettre1;
      LettreJoker[NumeroJoker[1]]:=TTypeJeton(Ord(tjA)+Ord(Lettre1)-Ord('A'));
      if NbJokers>1 then
        for Lettre2:='A' to 'Z' do
          begin
          stTirage[iJoker[2]]:=Lettre2;
          LettreJoker[NumeroJoker[2]]:=TTypeJeton(Ord(tjA)+Ord(Lettre2)-Ord('A'));
          Etape7;
          end
      else
        Etape7;
  ...
  begin
  stSolution:=Format(stFrmSolution, [Copy(stSol, PosSolution, Length(stSol)-PosSolution+1),
...
try
  with FormPatience do
    ...
    ProgressBar.Min:=0;
    ProgressBar.Max:=NbLettresMaxMot*NbLettresMaxMot;
...
try
  ...
  for i:=1 to Tour-1 do
    ...
    Write(f, Format(stFrmHTMLFdmdLgn,
                    [i, stHTMLTirage,
                     ...
                     PosPrp[i], stHTMLSuffixeRang[Ord(PosPrp[i]>1)], NbSol[i], // vLaz : stHTMLSuffixeRang au lieu de stSuffixeRang
                     ...
                     stHTMLSuffixeRang[Ord(CumulPosPrp[i] div i>1)], CumulNbSol[i] div i, // v1.4.7. vLaz : stHTMLSuffixeRang au lieu de stSuffixeRang
  ...
  with FormFeuilleMatch do // v1.5
    begin
    RichMemo.SelStart:=0; // v1.5
    RichMemo.SelLength:=0; // v1.5
...
case Nature of
  ...
  ntrTailleMot:   Result:=Format('%d lettres', [Valeur]);
...
begin
// 1. On stocke et on trie les mots de 15 lettres avec leurs scores
...
with slTemp do
  ...
  for i:=0 to n do
    ...
    for j:=1 to NbLettresMaxMot do
      Inc(Score, ValeurJeton[TTypeJeton(2+Ord(stMot[j])-Ord('A'))]);
    Add(Format('%d-%s (%d points)', [99-Score, stMot, Score]));
    end
  end;
// 2. On restocke les mots de 15 lettres où l'index est le n° de mot après tri
...
try
  with IniFile do
    ...
    WriteInteger(stSectionDefiM15L, stEntreeDefiM15LVersionDico+IntToStr(n), StrToInt(Copy(stVersionDico,4,1)));
    ...
    MessageBox(0,
               ...
               'Défi du mot de 15 lettres à former avec le moins de jetons possible', 0);
finally
  FreeAndNil(IniFile);
  GenereListeDefis(ExtractFilePath(ParamStr(0))+stNomFichierDefi1M15L, NAffichePas);
end{try}
end;
//---------------------------------------------------------------------------
  ...
  with IniFile do
    ...
    repeat
      ...
      if i<slMot.Count then
        for j:=1 to n do          // Défis relevés
          if StrToInt(Copy(stVersionDico,4,1))=ReadInteger(stSectionDefiM15L, stEntreeDefiM15LVersionDico+IntToStr(j), 0) then
            begin
...
initialization
  ...
  stNomFichierJournal:=ExtractFilePath(ParamStr(0))+'duplicata.log';
...
finalization
  ...
  if Journalisation then
    ...
    CloseFile(Journal);
    JournalOuvert:=False; // v1.5 : Empêche le Try CloseFile(Journal)
     OpenDocument(ExtractFileName(stNomFichierJournal)) { *Converti depuis ShellExecute* }
    end
//---------------------------------------------------------------------------
end.
