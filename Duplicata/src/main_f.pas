...
uses
  ...
  ActnList, Menus, ExtCtrls, Base, ComCtrls;
//---------------------------------------------------------------------------
type
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
 ...
 // procedure TimerClignotementTimer(Sender: TObject); // vLaz : Plus de clignotement
  ...
  private
    ...
    MontreCaseEnCours, // vLaz : paramètre MontreCase de DessineCase pour RedessineCase
    ...
    procedure DessineCoordonnees; // v1.3.9. vLaz : privée
    ...
    procedure DessinePlateau; // vLaz : On cache DessinePlateau pour obliger à utiliser TForm.Repaint
  ...
  public
    ...
    procedure RedessineCase(const X, Y : TCoordonnee; const MontreCase : Boolean); // vLaz : repaint une partie du plateau (en général une case à la fois)
...
var
  FormMain: TFormMain;
  stTitrePlateau                : String = 'Duplicata'; // v1.1 (const -> var) Pour changer l'initiale selon s'il y a mélange ou non
  DeltaHeight                   : Integer = 0; // v1.8.3 (défini à l'affichage du plateau)

//---------------------------------------------------------------------------
const
  CouleurBonus                  : array [TBonus] of TColor = ( $C0C0C0,   clAqua, $00FF4040, $C0C0FF, clRed); // v1.3.10 (bleu foncé éclairci)
...
implementation
//---------------------------------------------------------------------------
uses Math, StrUtils, IniFiles, LCLType, LCLIntf, LCLProc,
...
if AfficheMessage('Êtes-vous sûr de bien vouloir changer tout le tirage ?', // v1.5
                  stConfirmation,
...
if DemoEnCours or
   (AfficheMessage('Êtes-vous sûr de vouloir passer votre tour ?',
...
if stNomFichierPartie='' then
  if ActionOptionsGenererAutoFDM.Tag=0 then // v1.3.5
    stNomFichierFdMD:=ExtractFilePath(Application.ExeName)+'Duplicata.dup' // vLaz : ExeName à la place de Application.ExeName
  else
    begin
...
procedure TFormMain.ActionAffichageListeDefisReleves1M15LExecute(
  Sender: TObject);
begin // v1.5.3
d15l.GenereListeDefis(ExtractFilePath(Application.ExeName)+
...
// v1.5.6 : ajout du temps de réflexion (Partie/Duplicata)
// v1.8.4 : ajout "(64 bits)" après la version
const stFrmInfoPartie = 'Partie'#13#13' · Nom : %s.'#13' · Numéro : %s.'#13' · Version du format de fichier de partie : %3X.'#13' · Type de partie : %s.'#13' · Temps de réflexion : %s.'#13#13+
                        'Duplicata'#13#13' · Version : %s (64 bits)'#13' · Prochain n° de partie : %d.'#13' · Version courante du format de fichier de partie : %X.'#13' · Type de partie : %s.'#13' · Temps de réflexion : %s.';
begin // v1.5.3
...
begin
stNomFichierJournal:=ExtractFilePath(Application.ExeName)+'duplicata.log'; // vLaz : ExeName au lieu de Application.ExeName
...
if Journalisation then
  ...
  try
    ...
    NotifieAction(aanOptionsJournalRechercheSolutionsCAC, Inactif); // v1.5
     OpenDocument(ExtractFileName(stNomFichierJournal))
  except
  end
end;
//---------------------------------------------------------------------------
...
begin // v1.4.9
if AfficheMessage('Êtes-vous sûr de vouloir réinitialiser tous les records ?',
                  stConfirmation, // v1.5 (factorisation)
                  MB_YESNO+MB_ICONEXCLAMATION+MB_DEFBUTTON2)=IDYES then
  try
    IniFile:=TIniFile.Create(ExtractFilePath(Application.ExeName)+stNomFichierIni); // vLaz : ExeName au lieu de Application.ExeName
...
var IniFile : TIniFile;
begin // v1.4.9
if AfficheMessage('Êtes-vous sûr de vouloir vider la liste des parties jouées ?',
                  stConfirmation, // v1.5 (factorisation)
                  MB_YESNO+MB_ICONEXCLAMATION+MB_DEFBUTTON2)=IDYES then
  try
    IniFile:=TIniFile.Create(ExtractFilePath(Application.ExeName)+stNomFichierIni); // vLaz : ExeName au lieu de Application.ExeName
...
var FdMAffichee,            // v1.5
    ...
    fsRTF        : TFileStream; // vLaz : pour utiliser TRichMemo.SaveRichText
begin
...
try
  ...
  with FormFeuilleMatch do // v1.5 : au lieu de with FormFeuilleMatch.RichMemo.Lines
    begin
    ...
    try // v1.5
      ...
      // Deb vLaz : SaveRichText au lieu de Lines.SaveToFile
      (*
      RichMemo.Lines.SaveToFile(Copy(stNomFichierPartie,
                                1,
                                Length(stNomFichierPartie)-
                                Length(ExtractFileExt(stNomFichierPartie)))+'.rtf');
      *)
      try
        fsRTF := TFileStream.Create(Copy(stNomFichierPartie,
                                         1,
                                         Length(stNomFichierPartie)-
                                         Length(ExtractFileExt(stNomFichierPartie)))+'.rtf',
                                         fmCreate);
        RichMemo.SaveRichText(fsRTF);
      finally
        fsRTF.Free
      end;
      // Fin vLaz : SaveRichText au lieu de Lines.SaveToFile.
      InverseCouleurTexte; // v1.5. FdM : blanc sur fond noir. Fichier RTF : noir sur fond blanc
    finally // v1.5
...
if PartieModifiee and not MelangeEnCours then // v1.3.4 : Pas de confirmation si fin de partie (MelangeEnCours)
  begin
  if AfficheMessage('Une partie est en cours, êtes-vous sûr de vouloir abandonner et en commencer une nouvelle ?',
                    stConfirmation, // v1.5 (factorisation)
...
p.CommencePartie;
Repaint; // vLaz : Repaint remplace DessinePlateau
with FormTirage do
  ChargeTirage;
with FormFeuilleMatch do
  begin
...
if PartieModifiee and not MelangeEnCours and // v1.3.4 : Pas de confirmation si fin de partie (MelangeEnCours)
   (AfficheMessage('Une partie est en cours, êtes-vous sûr de vouloir abandonner et ouvrir une partie sauvegardée ?',
                   stConfirmation, // v1.5 (factorisation)
                   MB_YESNO+MB_ICONEXCLAMATION+MB_DEFBUTTON2)=IDNO) then
  begin
  ...
  for i:=Low(TCoordonnee) to High(TCoordonnee) do
    for j:=Low(TCoordonnee) to High(TCoordonnee) do
      if p.Proposition[i, j] then
        ...
        RedessineCase(i, j, NeMontrePasCase) // vLaz : RedessineCase remplace DessineCase
        end;
  Repaint // vLaz : on repaint tout le plateau !
end{try};
...
CanClose:=not PartieModifiee or MelangeEnCours or // v1.3.4 : Pas de confirmation si fin de partie (MelangeEnCours)
          (AfficheMessage('Une partie est en cours, êtes-vous sûr de vouloir abandonner ?',
                          stConfirmation, // v1.5 (factorisation)
...
DetermineVersion;
// EnregistreExtension; // v1.6.4 -> Fait dans l'installation de Duplicata (mode unique) permettant une compatibilité avec Windows Vista/Seven
stNomFichierPartie:='';
PartieModifiee:=False;
...
var x, y, xMin, xMax, yMin, yMax : Integer;
begin // vLaz : comme FormResize
if not Visible or
   RafraichissementEnCours then Exit; // Redimensionnement en cours
RafraichissementEnCours:=True;
try
  if Canvas.ClipRect.Left+Canvas.ClipRect.Top=0 then
    DessinePlateau
  else // On dessine les cases une par une
    with Canvas.ClipRect do
      begin
      xMin:=(Left-1-HauteurLargeurCoordonnees div 2) div TailleCase;
      xMax:=(Right-TailleCase-1-HauteurLargeurCoordonnees div 2) div TailleCase;
      yMin:=(Top-1-HauteurLargeurCoordonnees div 2) div TailleCase;
      yMax:=(Bottom-TailleCase-1-HauteurLargeurCoordonnees div 2) div TailleCase;
      for x:=xMin to xMax do
        for y:=yMin to yMax do
          RedessineCase(x, y, MontreCaseEnCours);
      end;
finally
  RafraichissementEnCours:=False;
end{with Canvas}
...
//---------------------------------------------------------------------------
(* vLaz : Clignotement retiré car oblige à rafraîchir le plateau
...
with TimerClignotement do
  ...
  if CoordonneesValides(xPopup, yPopup) then
    RedessineCase(xPopup, yPopup, Boolean(Tag)); // vLaz : RedessineCase remplace DessineCase
  end
end;
*)
//---------------------------------------------------------------------------
...
if (p<>nil) and (p.Dico<>nil) then
  ...
  if not p.Dico.ChargementDicoOk then
    ...
    if ParamCount>0 then // Si dico OK alors on charge la partie demandée en ligne de commande le cas échéant
      if FileExists(ParamStr(1)) then
        OuvrePartie(ParamStr(1))
      else
        AfficheMessage('Le fichier demandée est introuvable !', 'Impossible d''ouvrir la partie', MB_ICONHAND);
  end
end;
//---------------------------------------------------------------------------
...
begin
if MontreCase then
  begin (* // vLaz : Affichage sélection case suspendue car ne marche pas !
  if CoordonneesValides(xPopup, yPopup) then // On efface la mise en évidence précédente
    RedessineCase(xPopup, yPopup, NeMontrePasCase); // vLaz : RedessineCase remplace DessineCase *)
  xPopup:=X;
  yPopup:=Y
  end;
with Canvas do
  ...
  if (p=Nil) or (p.c[x, y]=0) then
   ...
   (* if MontreCase then // vLaz : Affichage sélection case suspendue car ne marche pas !
     // vLaz : pas de clignotement
     // Pen.Color:=TColor(TimerClignotement.Tag*CouleurChoixCase+(1-TimerClignotement.Tag)*Self.Color)
     Pen.Color:=CouleurChoixCase
   else *)
     Pen.Color:=Self.Color;
...
//---------------------------------------------------------------------------
procedure TFormMain.RedessineCase(const X,Y : TCoordonnee; const MontreCase : Boolean); // vLaz : repaint une partie du plateau (en général une case à la fois)
var InvRect : TRect;
begin
MontreCaseEnCours:=MontreCase;
InvRect:=Rect(1+TailleCase*X+HauteurLargeurCoordonnees div 2,
              1+TailleCase*Y+HauteurLargeurCoordonnees div 2,
              X+TailleCase,
              Y+TailleCase);
...
with Canvas do
 ...
 DessineCoordonnees; // v1.8.3 : on dessine les coordonnées avant le reste au cas où elles rongent les cases !
...
if MontreCase then
  RedessineCase(Colonne-1, Ligne-1, MontreCase); // Affecte xPopup et yPopup. vLaz : RedessineCase remplace DessineCase
...
for i:=0 to p.DistanceCaseDrnJetonPlace do
  RedessineCase(X+i*dx[d], Y+i*dy[d], NeMontrePasCase); // vLaz : RedessineCase remplace DessineCase
Repaint // vLaz : Oblige à redessiner tout le plateau
end;
//---------------------------------------------------------------------------
...
if NvStatutMelange then
  begin
  Caption:='D'+Copy(Caption, 2, Length(Caption)-1);
  stTitrePlateau[1]:='D'
  end
else
  begin
...
var i            : Integer;
    ...
    stMessage    : String;
    fsRTF        : TFileStream; // vLaz : pour utiliser TRichMemo.LoadRichText
begin
...
try
  ...
  with FormFeuilleMatch do // v1.5 : au lieu de FormFeuilleMatch.RichMemo.Lines
    begin
    FdMAffichee:=Visible; // v1.5
    if FdMAffichee then Visible:=False; // v1.5
    ...
    try
      ...
      // Deb vLaz : LoadRichText au lieu de Lines.LoadFromFile
      (*
      RichMemo.Lines.LoadFromFile(Copy(stNomFichierPartie,
                                       1,
                                       Length(stNomFichierPartie)-
                                       Length(ExtractFileExt(stNomFichierPartie)))+'.rtf');
      *)
      try
        fsRTF := TFileStream.Create(Copy(stNomFichierPartie,
                                         ...
                                         Length(ExtractFileExt(stNomFichierPartie)))+'.rtf',
                                         fmOpenRead);
        RichMemo.LoadRichText(fsRTF);
      finally
        fsRTF.Free
      end;
      // Fin vLaz : LoadRichText au lieu de Lines.SaveToFile
  ...
  Repaint;
  with FormFeuilleMatch.RichMemo.Lines do
    if Count=0 then
      begin
      Add(stEntete[0]); // v1.8.3 : plus qu'une seule taille
      Add(stEntete[1]); // v1.8.3 : plus qu'une seule taille
      if p<>nil then
...
stVersion:=Format('%d.%d.%d', [1,8,8]);
stSousVersion:=Format('.%d', [1]);
end;
//-----------------------------------------------------------------------------
...
if FormFeuilleMatch=nil then // v1.6.5 : Largeur FDM suivant version. Si feuille de match n'existe pas, on prend la version étroite (par défaut)
  FormMainClientWidth:=Screen.Width-LargeurClientFdMMin-Width+ClientWidth  // v1.8.3 : plus qu'une seule taille
else
  FormMainClientWidth:=Screen.Width-LargeurClientFdMMin-Width+ClientWidth; // v1.8.3 : plus qu'une seule taille
...
try
  ...
  with IniFile do
    ...
    if FormTirage<>nil then
      with FormTirage do
        begin
        DefinitDimensionsParDefaut; // v1.8.2 : plus de paramètres
        Top:=FormMain.Top;
...
try
  ...
  with IniFile do
    ...
    stNomFichier:=ExtractFilePath(Application.ExeName)+'Parties.html'; // vLaz : ExeName au lieu de Application.ExeName
...
function stHTMLLigneNatureRecord(const n : TNatureRecord) : string;
  ...
  case n of
    ntrNivDiff,
    ntrNbSolutions,
    // v1.6 : les deux natures de records ci-dessous
    ntrTpsCalcSol,
    ntrNbSolParSec: Result:=Format(stFrmHTMLLdR2LMin, [stHTMLNatureRecord[n], // v1.5.7 : ajout de tpStandard/tpJoker ci-dessous. vLaz : stHTMLNatureRecord au lieu de stNatureRecord
    ...
    Result:=Format(stFrmHTMLLdR1LMax, [stHTMLNatureRecord[n], // v1.5.7 : ajout de tpStandard/tpJoker ci-dessous. vLaz : stHTMLNatureRecord au lieu de stNatureRecord
...
try
  ...
  with IniFile do
    begin
    stNomFichier:=ExtractFilePath(Application.ExeName)+'Records.html'; // vLaz : ExeName au lieu de Application.ExeName
    AssignFile(f, stNomFichier);
    Rewrite(f);
